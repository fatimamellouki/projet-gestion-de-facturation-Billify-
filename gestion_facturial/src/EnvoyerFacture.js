...
export default function EnvoiFacture() {
  const { id } = useParams();
  ...
  const envoyerFacture = async (type = 'standard') => {
    setIsSending(true);
    setMessage(null);

    try {
      const endpoint = type === 'personnalise'
        ? `http://127.0.0.1:8000/api/factures/${id}/envoyer?modele=personnalise`
        : `http://127.0.0.1:8000/api/factures/${id}/envoyer?modele=standard`;
      ...
      setMessage({
        text: data.message || 'Facture envoyée avec succès !',
        type: 'success'
      });
    } catch (error) {
      setMessage({
        text: error.message || 'Erreur lors de l\'envoi de la facture',
        type: 'error'
      });
    } finally {
      setIsSending(false);
        ...
        <div className="p-6">
          <form className="space-y-6">
            ...
            <div className="flex space-x-4">
              <button
                type="button"
                onClick={() => envoyerFacture('standard')}
                disabled={isSending}
              ...
              <button
                type="button"
                onClick={() => envoyerFacture('personnalise')}
                disabled={isSending}
